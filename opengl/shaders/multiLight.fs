...
void main()
{
	...
	vec3 dirResult=CalcDirLight(directLight,norm,viewDir);
	vec3 pointResult=CalcPointLight(pointLight,norm,fragPos,viewDir);
	vec3 spotResult=CalcSpotLight(spotLight,norm,fragPos,viewDir);

	vec3 resultColor=(dirResult+pointResult+spotResult);
